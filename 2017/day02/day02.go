...
func Part1() {
	fmt.Println(CalculateSpreadsheetChecksum(INPUT02))
}

func Part2() {
	fmt.Println(CalculateSpreadsheetChecksumUsingDiv(INPUT02))
}

func CalculateSpreadsheetChecksumUsingDiv(input [][]int) int {
	sum := 0
	for _, line := range input {
		for i := 0; i < len(line); i++ {
			for j := i + 1; j < len(line)+i; j++ {
				if (line[i] % line[j%len(line)]) == 0 {
					sum += line[i] / line[j%len(line)]
				}
			}
		}
	}
...
func CalculateSpreadsheetChecksum(input [][]int) int {
	sum := 0
	for _, line := range input {
		max := math.MinInt64
		min := math.MaxInt64
